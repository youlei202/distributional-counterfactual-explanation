...
def bootstrap_sw(
    x, y, r=2, delta=0.1, alpha=0.05, B=1000, N=500, nq=500, N_fit=2000, theta=None
):
    ...
    for b in range(B):
        if resample:
            theta = np.random.multivariate_normal(
                np.repeat(0, d), np.identity(d), size=N
            )
            theta = np.apply_along_axis(lambda x: x / np.linalg.norm(x), 1, theta)
        ...
        boot.append(
            np.power(dist.sw(xx, yy, r=r, delta=delta, N=N, nq=nq, theta=theta), r)
            - SW_hat
        )
...
def pretest(
    x, y, r=2, delta=0.1, alpha=0.05, mode="DKW", N=500, B=500, nq=1000, theta=None
):
    ...
    if x.ndim == 1:
        ...
        C_exact = mc_sw(
            x, y, r=r, delta=delta, alpha=alpha / 2.0, N=N, nq=nq, theta=theta
        )
        arr_x = x
        arr_y = y
    ...
    return bootstrap_sw(
        x, y, r=r, delta=delta, alpha=alpha / 2.0, B=B, N=N, nq=nq, theta=theta
    )
