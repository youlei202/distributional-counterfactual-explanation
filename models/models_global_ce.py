...
def performance(model, x_train, x_test, y_train, y_test, normalise=None):
    ...
    print(
        "\033[1mTrain Accuracy:\033[0m {}%".format(
            round(metrics.accuracy_score(y_train, y_pred_tr) * 100, 2)
        )
    )
    print(
        "\033[1mTest Accuracy:\033[0m {}%".format(
    ...
    print(
        "\033[1mProportion of 1s Predicted (Train):\033[0m {}%".format(
            round(sum(y_pred_tr) / len(y_pred_tr) * 100, 2)
        )
    )
    print(
        "\033[1mProportion of 1s Predicted (Test):\033[0m {}%".format(
            round(sum(y_pred) / len(y_pred) * 100, 2)
        )
    )
...
class DNN_trainer:
    ...
    def train(self, num_epochs, patience, print_outputs=False):
        ...
        print(
            "Number of Steps per Epoch = ceil(Length of Trainset / Batch Size) = {}".format(
                self.total_step
            )
        )
            ...
            if self.val_ratio:
                ...
                if val_loss > last_val_loss:
                    ...
                    if early_stop_count >= patience:
                        ...
                        if self.test_loader:
                            self.test_losses[self.epoch] = self.validation_epoch(
                                self.test_loader
                            )
                            self.plot_progress(
                                self.losses[: self.epoch + 1],
